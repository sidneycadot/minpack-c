...
enum lm_info_t lmdif(lmdif_fcn_ptr fcn, void * fcn_context,
           const int m, const int n, double *x, double *fvec, const double ftol, const double xtol, const double gtol, const int maxfev,
           const double epsfcn, double *diag, const enum lm_mode_t mode, const double factor, int *nfev,
           double *fjac, const int ldfjac, int *ipvt, double *qtf, double *wa1, double *wa2, double *wa3, double *wa4)
{
    ...
    //   The function returns an info value. If the user has
    ...
    *nfev = 0;
    ...
    (*fcn)(m, n, x, fvec, LM_IFLAG_1, fcn_context);
    *nfev = 1;
    ...
    for (;;) // Outer loop.
    {
        ...
        fdjac2(fcn, fcn_context, m, n, x, fvec, fjac, ldfjac, epsfcn, wa4);
        *nfev += n;

        // Compute the QR factorization of the Jacobian.

        qrfac(m, n, fjac, ldfjac, ipvt, wa1, wa2, wa3);
        ...
        for (;;) // Inner loop.
        {
            ...
            (*fcn)(m, n, wa2, wa4, LM_IFLAG_1, fcn_context);
            ++(*nfev);
            ...
            const bool crit1 = fabs(actred) <= ftol && prered <= ftol && p5 * ratio <= 1.0;
            const bool crit2 = delta <= xtol * xnorm;
